...
Animate::Animate(const std::string& texturePath, const int& cols, const int& rows, const float& widthI, const float& heightI )
...
void Animate::divideSheet()
{
	if (m_cols == 0 || m_rows == 0) 
		return;

	m_widthSpriteWithDeadZone = static_cast<float>(m_widthI) / m_cols;
	m_heightSpriteWithDeadZone = static_cast<float>(m_heightI) / m_rows;
}
...
void Animate::firstSpriteCoord()
{
	m_topLeft.x = 0 + m_widthDeadZone;
	m_topLeft.y = m_heightI - m_heightDeadZone;
}


void Animate::lastSpriteCoord()
{
	m_bottomRight.x = m_widthI - m_widthDeadZone;
	m_bottomRight.y = 0 + m_heightDeadZone;
}
